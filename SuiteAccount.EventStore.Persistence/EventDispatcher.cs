...
using SuiteAccount.Infrastructure.Core;

namespace SuiteAccount.EventStore.Persistence
{
    public class EventDispatcher
    {
        ...
        private void RecoverSubscription()
        {
            ...
            this._eventStoreSubscription = SubscribeToAll();
        ...
        {
            this._stop = true;
            if (this._eventStoreSubscription != null)
            ...
            if (!_historicalDone.Wait(ThreadKillTimeoutMillisec))
                throw new TimeoutException("Unable to stop dispatching in time.");

            if (!_liveDone.Wait(ThreadKillTimeoutMillisec))
                throw new TimeoutException("Unable to stop dispatching in time.");
        }
        ...
        private EventStoreSubscription SubscribeToAll()
        {
            ...
            var task = this._eventStoreConnection.SubscribeToAllAsync(false, EventAppeared, SubscriptionDropped);
        ...
        private void EventAppeared(EventStoreSubscription eventStoreSubscription, ResolvedEvent resolvedEvent)
        {
            ...
            _liveQueue.Enqueue(resolvedEvent);

            //Prevent live queue memory explosion.
            if (!_livePublishingAllowed && _liveQueue.Count > LiveQueueSizeLimit)
            {
        ...
        private void PublishEvents(ConcurrentQueue<ResolvedEvent> queue, ManualResetEventSlim doneEvent)
        {
            ...
            while (keepGoing)
            {
                ...
                keepGoing = !this._stop && queue.Count > 0 && Interlocked.CompareExchange(ref _isPublishing, 1, 0) == 0;
            }
        }
        ...
        private static EventBase DeserializeEvent(byte[] metadata, byte[] data)
        {
            ...
            var eventBase = (EventBase)JsonConvert.DeserializeObject(Encoding.UTF8.GetString(data), Type.GetType((string)eventClrTypeName));
