...
using SuiteAccount.EventStore.Persistence.Exceptions;
    ...
    public class EventStoreRepository : IRepository
    {
        ...
        private void StreamNameFactory(IAggregate aggregate, Guid aggregateId)
        {
            this._aggregateIdToStreamName =
                (t, g) =>
                    string.Format("{0}{1}-{2}", char.ToLower(aggregate.GetType().Name[0]),
                        aggregate.GetType().Name.Substring(1), aggregateId.ToString("N"));
        }
        ...
        public TAggregate GetById<TAggregate>(Guid id, int version) where TAggregate : class, IAggregate
        {
            ...
            do
            {
                ...
                currentSlice = _eventStoreConnection.ReadStreamEventsForwardAsync(streamName, sliceStart, sliceCount,
                    false).Result;
